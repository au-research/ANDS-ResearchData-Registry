...
function ViewCtrl($scope, $routeParams, ds_factory, $location, $timeout) {
  ...
  /**
	 * Binding the socket.io interface
	 * graceful degrade if socket is not defined well
   */
	$scope.bindSocket = function() {

		if (typeof socket_url === 'undefined' || socket_url === "" || typeof io === "undefined") return;

    $scope.socket = io(socket_url, {transports: ['websocket', 'polling', 'flashsocket']})

		// update datasource.id.harvest on message
    $scope.socket.on('datasource.' + $scope.ds.id + '.harvest', function (msg) {
      try {
        $scope.harvester = JSON.parse(msg)
		...
		// update datasource.id.log on message
